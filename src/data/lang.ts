const privateLang = {
  en: {
    ...
    "Ecitty Desc": "Developed the social network ecitty.com: geo-located post feed, real-time messenger, subscription system, push notifications, admin moderation panel.<br><br><b>Process:</b> Scrum, 2-week sprints. Team: 8 people (2 front, 3 back, designer, QA, analyst/project manager)<br><br><b>Results:</b><ol><li>Designed and implemented a GraphQL API from scratch: 40+ queries, 60+ mutations, 9 real-time subscriptions (WebSocket), cursor pagination, Apollo Persisted Queries with Redis caching;</li><li>Designed the monorepo architecture (Lerna) with 3 NestJS microservices and 20+ MongoDB collections with optimized indexes (GeoJSON, TTL, soft-delete);</li><li>Built a real-time messenger (frontend + backend): private and group chats, delivery and read statuses, typing indicator, reactions, user blocking, push notifications via Firebase Cloud Messaging;</li><li>Developed the geo-located post feed on React and Apollo Client: publication editor with media upload to S3 (pre-signed URL), Google Maps API with geo-search of posts (MongoDB 2dsphere), reactions and comments system;</li><li>Implemented the full authentication cycle: JWT with automatic access/refresh token rotation, OAuth 2.0 (Google, Facebook), SMS verification (SMSC.ru), WhatsApp OTP. Introduced 3-level rate-limiting and brute-force protection;</li><li>On the frontend, introduced MobX (18 stores), PWA with Workbox (precaching, offline, push notifications via Service Worker), code splitting (React.lazy), Sentry monitoring, feature-based architecture with 22+ path aliases;</li><li>Ensured security (Helmet, throttling, AES-256-GCM encryption, anti-spam protection), wrote 100+ unit tests (Jest), set up CI/CD on GitHub Actions and deployment via Docker Compose + Nginx + SSL;</li><li>Implemented localization for 3 languages (ru, en, kk) on both layers (i18next + Accept-Language), email campaigns (Nodemailer), a report system with an admin moderation panel, online presence via heartbeat.</li></ol><b>Stack:</b> React, NestJS, TypeScript, GraphQL, Apollo Client/Server, MobX, MongoDB, Redis, Docker, Nginx, GitHub Actions, Firebase FCM, AWS S3, Passport.js, Google Maps API, WebSocket, Workbox (PWA), Sentry, Jest, Lerna, i18next",
    "Cupcake Development Desc": "Developed commercial products for international clients in a distributed team: logistics platform, browser extensions, food delivery mobile app, cart-sharing SaaS service.<br><br><b>Results:</b><ol><li>Built the backend of a logistics platform on NestJS + Fastify + Drizzle ORM (PostgreSQL): air cargo tariff calculation engine with Excel parsing using the Strategy pattern, AmoCRM integration (OAuth2, deal import/export), async route optimization via Yandex MQ + S3, currency conversion via the Central Bank of Russia. Deployed to Yandex Serverless Containers;</li><li>Developed two cross-browser extensions (Chrome, Firefox, Safari) on React + TypeScript + Vite/Webpack (Manifest V3): a cashback extension injecting badges into search results (Google/Bing/Yahoo) via Shadow DOM, and a cart-sharing extension supporting 286 retailers (Amazon, Walmart, IKEA and others) via vendor-specific DOM scrapers;</li><li>Built a food delivery mobile app on React Native (iOS + Android): catalog, cart, online payment (card, SBP), Yandex Maps (address selection, delivery zones), real-time order tracking via WebSocket, push notifications (FCM + OneSignal), loyalty program, promo codes, stories;</li><li>Performed mobile app performance optimization: memoization (React.memo, useMemo, useCallback), refactoring of the context and cart architecture, unit tests for hooks, API, contexts and components;</li><li>Developed a React SPA (Jotai, TanStack Query, MUI, Tailwind) and an Express.js backend (PostgreSQL, Firebase, Redis, Stripe subscriptions, OAuth via Google/Facebook/Apple/Microsoft) for a cart-sharing SaaS service with retailer API integrations (Amazon PAAPI, Walmart, eBay, Kroger);</li><li>Configured CI/CD on GitHub Actions for multiple projects: Yandex Cloud deployment, Docker image builds, DB migrations, Swagger documentation, Sentry monitoring, CodePush OTA updates.</li></ol><b>Stack:</b> NestJS, Fastify, React, React Native, TypeScript, PostgreSQL, Drizzle ORM, Firebase, Redis, Express.js, Vite, Webpack, Tailwind CSS, MUI, Jotai, TanStack Query, Stripe, Docker, GitHub Actions, Yandex Cloud, Chrome Extensions (MV3), Yandex Maps, Sentry, Swagger, Jest, Puppeteer",
  } as any,
  ru: {
    "contacts": "контакты",
    "about": "обо мне",
